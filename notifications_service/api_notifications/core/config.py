...
class Settings(BaseSettings):
    ...
    project_name: str = Field('notification', env='NOTIFY_PROJECT_NAME')
    ...
    auth_url: str = Field('http://auth:8001/api/v1/authorizate', env='AUTH_URL')
    db_type = Field('postgresql', env='DB_TYPE')
    postgres_user: str = Field('user', env='POSTGRES_USER')
    postgres_password: str = Field('stoppush', env='POSTGRES_PASSWORD')
    postgres_host: str = Field('db', env='POSTGRES_HOST')
    postgres_port: int = Field(5432, env='POSTGRES_PORT')
    postgres_db: str = Field('base', env='POSTGRES_DB')
